...
func TestResumeCronWorkflowTool(t *testing.T) {
	tool := ResumeCronWorkflowTool()

	assert.Equal(t, "resume_cron_workflow", tool.Name)
	assert.NotEmpty(t, tool.Description)
	assert.Contains(t, tool.Description, "Resume")
	assert.Contains(t, tool.Description, "CronWorkflow")
}

func TestResumeCronWorkflowInput(t *testing.T) {
	// Test default values
	input := ResumeCronWorkflowInput{}
	assert.Empty(t, input.Namespace)
	assert.Empty(t, input.Name)

	// Test with values
	input2 := ResumeCronWorkflowInput{
...
func TestResumeCronWorkflowOutput(t *testing.T) {
...
func TestResumeCronWorkflowHandler(t *testing.T) {
	tests := []struct {
		setupMock func(*mocks.MockCronWorkflowServiceClient)
		validate  func(*testing.T, *ResumeCronWorkflowOutput, *mcp.CallToolResult)
		name      string
		input     ResumeCronWorkflowInput
		wantErr   bool
	}{
		{
			name: "success - resume cron workflow",
			input: ResumeCronWorkflowInput{
				Namespace: "default",
				Name:      "my-cron",
			},
			setupMock: func(m *mocks.MockCronWorkflowServiceClient) {
				m.On("ResumeCronWorkflow", mock.Anything, mock.MatchedBy(func(req *cronworkflow.CronWorkflowResumeRequest) bool {
			...
			wantErr: false,
			validate: func(t *testing.T, output *ResumeCronWorkflowOutput, result *mcp.CallToolResult) {
				assert.Equal(t, "my-cron", output.Name)
				assert.Equal(t, "default", output.Namespace)
				assert.Equal(t, []string{"0 * * * *"}, output.Schedules)
				assert.False(t, output.Suspended)
				assert.Contains(t, output.Message, "resumed successfully")
				...
				assert.Contains(t, text.Text, "resumed successfully")
				assert.Contains(t, text.Text, "active")
			},
		},
		{
			...
			wantErr: false,
			validate: func(t *testing.T, output *ResumeCronWorkflowOutput, _ *mcp.CallToolResult) {
				assert.Equal(t, "my-cron", output.Name)
		...
		},
		{
			...
			setupMock: func(m *mocks.MockCronWorkflowServiceClient) {
				m.On("ResumeCronWorkflow", mock.Anything, mock.Anything).Return(
					nil,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			handler := ResumeCronWorkflowHandler(mockClient)
			ctx := t.Context()
